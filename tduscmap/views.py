...
def get_friend_favorites(request, friend_id):
    """Récupérer les favoris d'un ami."""
    user = request.user
    try:
        friend = user.friends.get(id=friend_id)  # Assurez-vous que cela fonctionne avec votre modèle
        favorites = Favorite.objects.filter(user=friend)  # Filtrer les favoris par ami
        favorites_list = [{'lat': favorite.lat, 'lng': favorite.lng, 'description': favorite.description} for favorite in favorites]
        return JsonResponse(favorites_list, safe=False)
